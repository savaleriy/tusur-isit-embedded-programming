...
Что такое класс, спецификаторы доступа, конструкторы, деструкторы и жизненный цикл объекта.
...
- переменные (также называемые "fields"),
...
Порядок событий в жизни объекта задан языком и не зависит от программиста:

```mermaid
graph TD
    A["Выделено место<br/><i>стек, статическая память или куча</i>"]
    B["Конструкторы полей<br/><i>в порядке объявления</i>"]
    C["Тело конструктора"]
    D["Объект живёт<br/><i>вызовы методов</i>"]
    E["Тело деструктора"]
    F["Деструкторы полей<br/><i>в обратном порядке</i>"]
    G["Память освобождена"]
    A --> B --> C --> D --> E --> F --> G
```

Два следствия, из-за которых чаще всего ошибаются:

- **Поля инициализируются в порядке объявления в классе**, а не в порядке, записанном в списке инициализации. Если поля зависят друг от друга, порядок в классе и определяет результат.
- **Деструкторы вызываются в обратном порядке.** Объект, созданный последним, уничтожается первым.
#### Конструктор
