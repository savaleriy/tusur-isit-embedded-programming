...
Интерфейсы, управление временем жизни объектов, семантика копирования.
...
Интерфейс задаёт, **что** объект умеет, ничего не говоря о том, **как** он это делает. Код, работающий через интерфейс, не знает конкретного типа:
...
classDiagram
    ...
    IDisplay <|.. LCD1602
    IDisplay <|.. TFT_ILI9341
    IDisplay <|.. SerialDisplay
    Приложение ..> IDisplay : работает только с интерфейсом
```

Замена дисплея в такой программе - это замена одной строки создания объекта. Прикладной код не меняется вовсе.

> **На микроконтроллере** такой интерфейс обходится в указатель на таблицу виртуальных функций - 2 байта на каждый объект - плюс сама таблица во Flash. Когда реализация одна, это лишний расход, и интерфейс не нужен. Когда их несколько и они действительно взаимозаменяемы, цена оправдана. Третий путь - шаблон: он даёт ту же подстановку без затрат во время работы, но требует, чтобы тип был известен при компиляции.
