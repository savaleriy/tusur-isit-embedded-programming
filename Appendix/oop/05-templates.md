...
Шаблонные функции, вывод параметров, шаблонные структуры и классы.
...
Шаблон - это не функция, а **инструкция компилятору, как её изготовить**. Пока шаблон не вызван ни с одним типом, кода нет:

```mermaid
graph LR
    T["<b>Шаблон</b><br/>template T max(T a, T b)"]
    C1["Вызов max(1, 2)"]
    C2["Вызов max(1.5, 2.5)"]
    C3["Вызов max(c1, c2)"]
    I1["int max(int, int)"]
    I2["double max(double, double)"]
    I3["Celsius max(Celsius, Celsius)"]
    T --> C1 --> I1
    T --> C2 --> I2
    T --> C3 --> I3
```

Каждая инстанциация - **отдельная копия машинного кода**. На настольном компьютере это не имеет значения, на ATmega328P с 32 КБ Flash - имеет: шаблон, применённый к пяти типам, занимает место пяти функций. Отсюда практическое правило для микроконтроллера: шаблоны хороши там, где они заменяют дублирование кода, и вредны там, где порождают его.
