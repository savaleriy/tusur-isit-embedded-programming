# Отношения между классами и полиморфизм

Глубокое и поверхностное копирование, композиция, агрегация, ассоциация, зависимость, полиморфизм.
...
Пять способов связать классы, от самой прочной связи к самой слабой:
...
classDiagram
    ...
    class Ассоциация
    class Зависимость
    Наследование <|-- Композиция : слабее
    Композиция <|-- Агрегация : слабее
    Агрегация <|-- Ассоциация : слабее
    Ассоциация <|-- Зависимость : слабее
```

| Отношение | Формулировка | Кто отвечает за время жизни | Обозначение UML |
| --- | --- | --- | --- |
| Наследование | "является" | - | пустой треугольник |
| Композиция | "состоит из", часть не существует отдельно | владелец создаёт и уничтожает | закрашенный ромб |
| Агрегация | "содержит", часть существует отдельно | кто-то снаружи | пустой ромб |
| Ассоциация | "знает о" | никто из двоих | простая линия |
| Зависимость | "использует в одном методе" | никто | пунктирная стрелка |

На языке UML те же пять связей выглядят так:

```mermaid
classDiagram
    direction LR
    Автомобиль *-- Двигатель : композиция
    Выставка o-- Картина : агрегация
    Студент --> Группа : ассоциация
    Отчёт ..> Принтер : зависимость
    Транспорт <|-- Автомобиль : наследование
```

Практическое правило: если удаление владельца обязано уничтожить и часть - это композиция; если часть переживёт владельца - агрегация; если объект лишь упоминается в сигнатуре одного метода - зависимость.
